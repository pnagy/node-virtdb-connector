...
Diag = (function() {
  ...
  Diag._getNewSymbols = function() {
    var Symbols, i, len, ref, symbol;
    Symbols = [];
    ref = Diag._newSymbols;
  ...
  Diag._getNewHeaders = function() {
    var Headers, header, i, len, ref;
    Headers = [];
    ref = Diag._newHeaders;
    for (i = 0, len = ref.length; i < len; i++) {
      header = ref[i];
      Headers.push(header);
    }
  ...
  Diag._clearSentHeadersAndSymbols = function() {
    Diag._newHeaders = [];
    return Diag._newSymbols = [];
  };
  ...
  Diag._createDiagServiceMessage = function(level, args) {
    var argument, i, len, record, type;
    record = {};
  ...
  Diag.log = function(level, args) {
    var isSendingSuccessful;
    isSendingSuccessful = Protocol.sendDiag(Diag._createDiagServiceMessage(level, args));
    if (isSendingSuccessful) {
      Diag._clearSentHeadersAndSymbols();
    }
    if ((Diag.isConsoleLogEnabled === true) || (!isSendingSuccessful)) {
